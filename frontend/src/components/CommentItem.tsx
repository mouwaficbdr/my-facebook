...
export default function CommentItem({
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      error(
        err instanceof Error ? err.message : 'Erreur lors de la gestion du like'
      );
    } finally {
    ...
    <div className="flex space-x-3">
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center space-x-4 mt-1 px-3">
          ...
          >
            <Heart
              className={`w-3 h-3 ${localUserLiked ? 'fill-current' : ''}`}
            />
      ...
      <ConfirmModal
        ...
        message={
          deleteTarget?.isReply
            ? `Êtes-vous sûr de vouloir supprimer cette réponse ?\n\n"${deleteTarget.content.substring(
                0,
                50
              )}${deleteTarget.content.length > 50 ? '...' : ''}"`
            : `Êtes-vous sûr de vouloir supprimer ce commentaire ?\n\n"${deleteTarget?.content.substring(
                0,
                50
              )}${
                deleteTarget && deleteTarget.content.length > 50 ? '...' : ''
